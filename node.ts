...
export function startNode(gatewayUrl: string = 'ws://localhost:8080') {
  const ws = new WebSocket(gatewayUrl);
  const hal = new NodeHardwareLayer();
  const speaker = hal.getSpeaker(16000);
  ...
  keyListener.addListener((e, down) => {
    // Trigger on Left Ctrl + Left Shift + L
    if (e.state === "DOWN" && e.name === "L" && down["LEFT CTRL"] && down["LEFT SHIFT"]) {
