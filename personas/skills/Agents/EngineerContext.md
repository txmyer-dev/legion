...
- **skills/SNAP/CONSTITUTION.md** - Constitutional principles
...
- **Security** → skills/SNAP/SecurityProtocols.md
...
8. Write at least 1 integration test covering this issue's changes in `tests/`
...
**Every issue MUST include at least 1 integration test.**

- Tests go in the project's `tests/` directory
- Use the appropriate subdirectory: `unit/`, `integration/`, `e2e/` (web apps) or `skills/`, `hooks/`, `tools/`, `system/` (SNAP infra)
- Each test file starts with a comment header:
  ```
  # [test-name].test.sh
  # Covers: Issue #N — [brief description]
  # Validates: [what this test proves]
  ```
- Tests must be runnable independently (`bash test.sh` or `bun test file.test.ts`)
- The IntegrationTester agent will run ALL tests after you mark "Ready for QA"
- If your tests fail, the issue comes back to you — not to QA
