...
The Agents skill uses the standard SNAP SYSTEM/USER two-tier pattern:

| Location | Purpose | Updates With SNAP? |
|----------|---------|-------------------|
| `Data/Traits.yaml` | Base traits, example voices | Yes |
| `USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml` | Your voices, prosody, agents | No |

**How it works:** ComposeAgent.ts loads base traits, then merges user customizations over them. Your customizations are never overwritten by SNAP updates.

### User Customization Directory

Create your customizations at:
```
~/.claude/skills/SNAP/USER/SKILLCUSTOMIZATIONS/Agents/
