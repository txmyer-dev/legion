...
/**
 * Load environment variables from ${SNAP_DIR}/.env
 * This ensures API keys are available regardless of how the CLI is invoked
 */
async function loadEnv(): Promise<void> {
  const paiDir = process.env.SNAP_DIR || resolve(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude');
...
// SNAP directory for documentation paths
const SNAP_DIR = process.env.SNAP_DIR || resolve(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude');
...
MORE INFO:
  Documentation: ${SNAP_DIR}/skills/Art/README.md
  Source: ${SNAP_DIR}/skills/Art/Tools/Generate.ts
`);
  process.exit(0);
}
...
async function main(): Promise<void> {
  try {
    // Load API keys from ${SNAP_DIR}/.env
    await loadEnv();
