# SNAP Pack Icon Workflow

**Generate 256x256 transparent PNG icons for SNAP packs.**
...
Create consistent, professional icons for SNAP packs following the established visual identity.
...
3. **Consistent style** - Match existing SNAP pack icons
...
bun run ~/.claude/skills/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/SNAP/Packs/icons/[PACK_NAME].png
```
...
ls -la ${PROJECTS_DIR}/SNAP/Packs/icons/[PACK_NAME].png

# Check dimensions (requires imagemagick)
file ${PROJECTS_DIR}/SNAP/Packs/icons/[PACK_NAME].png
```
...
bun run ~/.claude/skills/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/SNAP/Packs/icons/pai-hook-system.png
```
...
bun run ~/.claude/skills/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/SNAP/Packs/icons/pai-core-install.png
```
...
bun run ~/.claude/skills/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/SNAP/Packs/icons/pai-memory-system.png
```
...
All SNAP pack icons go to:
```
${PROJECTS_DIR}/SNAP/Packs/icons/[PACK_NAME].png
```
...
- [ ] **Exists** at `${PROJECTS_DIR}/SNAP/Packs/icons/[PACK_NAME].png`
...
- [ ] **Consistent** - matches other SNAP icons in style

---

## Related Workflows

- The SNAP skill's CreateRelease workflow (may include icon generation)
