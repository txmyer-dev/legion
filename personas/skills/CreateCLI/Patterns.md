...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: API_KEY not found in ${SNAP_DIR}/.env');
    ...
    console.error('Error: Cannot read ${SNAP_DIR}/.env');
    console.error('Create file: touch ${SNAP_DIR}/.env');
    process.exit(1);
  }
}
```
...
**Key principles:**
- Load from ${SNAP_DIR}/.env (SNAP standard)
...
CONFIGURATION:
  API Key: ${SNAP_DIR}/.env (API_KEY=your_key)
  Base URL: ${DEFAULTS.baseUrl}

PHILOSOPHY:
  ${CLI_NAME} follows SNAP's CLI-First Architecture:
...
- [ ] Configuration loading (from ${SNAP_DIR}/.env)
