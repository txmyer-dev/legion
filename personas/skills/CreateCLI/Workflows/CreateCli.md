...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: {{ENV_VAR_NAME}} not found in ${SNAP_DIR}/.env');
      process.exit(1);
    }
    ...
    console.error(`Error: Cannot read ${SNAP_DIR}/.env file`);
    console.error('Make sure {{ENV_VAR_NAME}} is set in ${SNAP_DIR}/.env');
    process.exit(1);
  }
}
```
...
**Reference:** `~/.claude/skills/SNAP/CliFirstArchitecture.md` (Configuration Flags section)
...
Next steps:
1. Configure: Add {{ENV_VAR_NAME}} to ${SNAP_DIR}/.env
...
Next steps:
1. Add NOTION_API_KEY=your_key to ${SNAP_DIR}/.env
