...
**Comprehensive guide to using paid OSINT services in SNAP webassessment skill**
...
Add your API keys to `${SNAP_DIR}/.env`:
...
**CRITICAL:** Ensure `${SNAP_DIR}/.env` is in `.gitignore` and NEVER commit it to any repository.
...
**Error:**
```
APIKeyMissingError: Shodan API key not found. Set SHODAN_API_KEY in ${SNAP_DIR}/.env
```

**Solution:**
1. Verify API key is in `${SNAP_DIR}/.env`
...
2. **Configure API Keys**: Add keys to `${SNAP_DIR}/.env`
...
**Key Takeaways:**
- ✅ All three services require API keys (configure in `${SNAP_DIR}/.env`)
