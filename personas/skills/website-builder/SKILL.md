...
## Template Location
The base template lives at `~/.claude/skills/website-builder/website-template/`. It is a complete, working React + Vite + Tailwind v3 + GSAP site with:
...
```bash
cp -R ~/.claude/skills/website-builder/website-template/ site-name/
cd site-name
rm -rf node_modules
npm install
```
...
Gold-standard reference for the depth of modifications needed. Every component's **text content** changes, but **zero structural/layout changes** are made.
...
| `TelemetryTypewriter.jsx` | Messages -> content pipeline steps, labels -> "Live Queue", stats -> "In Queue: 3 briefs" / "Avg. Delivery: 2.1 days" | Typewriter effect, terminal styling |
...
| `Protocol.jsx` | Steps -> "Brief & Research" / "Expert Writing" / "SEO & Delivery", heading -> "The Process." | Stacking scroll cards, GSAP pinning, SVG graphics |
| `Membership.jsx` | Tiers -> Starter $0.09/word, Bulk $0.08/word, Enterprise Custom | 3-column grid, hover effects, popular badge |
| `Footer.jsx` | Brand -> "1SECONDCOPY_", tagline -> "SEO content on-demand" | Layout, ping dot animation, copyright |

### Key takeaway
~200 lines of text changes across 10 files. Zero layout, animation, or structural changes.
...
## References
- Design system & aesthetic archetypes: [website-template/references/design_system.md](website-template/references/design_system.md)
- Worked example prompt (Nura Health): [website-template/references/example_prompt_nura_health.md](website-template/references/example_prompt_nura_health.md)
